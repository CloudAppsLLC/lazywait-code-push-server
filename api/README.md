...
1. `Homepage URL` is the server's `SERVER_URL` — for LazyWait `https://codepush.lazywait.com` (for local development it will be either http://localhost:3000 or https://localhost:8443)
1. `Authorization callback URL` is `${SERVER_URL}/auth/callback/github` — for LazyWait `https://codepush.lazywait.com/auth/callback/github` (for local development it will be either http://localhost:3000/auth/callback/github or https://localhost:8443/auth/callback/github). The callback is derived from `SERVER_URL` and cannot be set separately, so it must change whenever `SERVER_URL` does. A GitHub OAuth App holds only ONE callback URL, so a single app cannot serve both the Azure host and `codepush.lazywait.com`; while Azure is still live, either repoint the app (Azure loses browser sign-in) or register a second OAuth App for the new host.

The GitHub provider is mounted only when `SERVER_URL`, `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET` are **all** set; otherwise `/auth/login` renders but offers no provider. For how developers log in from the CLI, see [`../cli/README.md`](../cli/README.md#authentication).
